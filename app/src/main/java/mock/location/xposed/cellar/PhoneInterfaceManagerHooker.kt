...
import mock.location.xposed.helpers.reflect.isPublic
...
class PhoneInterfaceManagerHooker {
            ...
            after { param ->
                ...
                try {
                    if (profile.describesCell) {
                        cells.add(mock.location.xposed.cellar.info.Lte().cellInfo(profile))
                    }
                } catch (t: Throwable) {
    ...
    private fun spoofedCells(profile: Profile): ArrayList<CellInfo> = ArrayList<CellInfo>().apply {
        try {
            if (profile.describesCell) {
                add(mock.location.xposed.cellar.info.Lte().cellInfo(profile))
            }
        } catch (t: Throwable) {
