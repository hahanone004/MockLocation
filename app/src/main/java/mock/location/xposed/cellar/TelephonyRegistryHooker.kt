...
import mock.location.xposed.cellar.info.DisplayInfo
...
class TelephonyRegistryHooker {
    ...
    fun hookListen(classLoader: ClassLoader) {
        ...
        fun spoofFor(callback: Any): mock.location.app.ui.models.Profile? {
        ...
        hookBeforeOnce(infoCallbacks) { param ->
            val profile = spoofFor(param.thisObject) ?: return@hookBeforeOnce
            param.args[0] = try {
                mutableListOf<CellInfo>(mock.location.xposed.cellar.info.Lte().cellInfo(profile))
            } catch (t: Throwable) {
